...
### Fixed
- Cooldown ON↔OFF só vale para **automação**. Demanda manual do painel (`panel_demand`), HTTP (`http_demand`), UI e botões físicos passa na hora.
- Após demanda manual, os timers de histerese são zerados para a automação não religar/desligar imediatamente.
